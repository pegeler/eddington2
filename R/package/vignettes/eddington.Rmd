...
ridden _E_ miles in _E_ days. So to get a number of 30, you need to have ridden
30 miles or more on 30 separate days.
...
number is in dark red---we'll see how to calculate that in the next section.
...
To see how the Eddington number progressed over the year, use `E_cum()`. We'll
want to add that as a new column onto the existing dataset:
...
increment the _E_ to 30. For that, we use `E_req()`, which stands for
"required."
